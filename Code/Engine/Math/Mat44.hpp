...
public:
	...
	explicit Mat44( Vec4 const& iBasis3D, Vec4 const& jBasis4D, Vec4 const& kBasis4D, Vec4 const& translation4D );
	...
	static Mat44 const MakeCameraToRenderTransform();
	static Mat44 const MakeLookAtTransform( Vec3 const& position, Vec3 const& targetPosition );

	Vec2 const    TransformVectorQuantity2D( Vec2 const& vectorQuantityXY ) const;   // assume z=0, w=0   no translation
	Vec3 const    TransformVectorQuantity3D( Vec3 const& vectorQuantityXYZ ) const;  // assume w=0        no translation
	Vec2 const    TransformPosition2D( Vec2 const& positionXY ) const;               // assume z=0, w=1
	Vec3 const    TransformPosition3D( Vec3 const& position3D ) const;               // assume w=1
