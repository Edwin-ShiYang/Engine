...
//-----------------------------------------------------------------------------------------------
void FloatRange::SetFromText( char const* text, char delimiter )
{
	Strings result = SplitStringOnDelimiter( text, delimiter );
	...
	m_min = static_cast< float > ( atof( result [ 0 ].c_str() ) );
	m_max = static_cast< float > ( atof( result [ 1 ].c_str() ) );
}
...
{
	if( m_max < floatRange.m_min || m_min > floatRange.m_max)
	{
		return false;
	}
	return true;
