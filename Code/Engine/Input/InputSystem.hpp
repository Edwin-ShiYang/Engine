...
public:
	...
	void UpdateLastFrameControllersButtons();

	void SetCursorMode( CursorMode currentMode );
	Vec2 GetCursorClientDelta() const;
	Vec2 GetCursorClientPosition() const;
	// Vec2 GetCursorNormalizedPosition() const;

	IntVec2 CursorScreenToClient();

	static bool Event_KeyPressed( EventArgs& args );
	static bool Event_KeyReleased( EventArgs& args );

private:
	void CorrectCursorVisibility();
	bool IsCursorVisible();

	IntVec2 m_currentCursorPos = IntVec2( 0, 0 );
	IntVec2 m_previousCursorPos = IntVec2( 0, 0 );;
